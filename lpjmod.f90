...
!  write(0,*)'flag D3b'
  ...
  if (doinsect) then
    ...
    if (insectmass(1,1,1) == 0. .and. insectenergy(1,1,1) ==0. .and. leafmass > 8500.) then
      insectmass(1,:,:)   = 0.063
      insectenergy(1,:,:) = 2.1571  !starting calories for L2o
	else
	 print*,"fried bananas please"
    end if
				...
				do m = 1,12
					...
					if (insectmass(8,1,g) == 0. .and. neggi(g) > 0. .and. insectmass(9,1,g)==0.) then ! hiro insectmass(8,1,g) == 0. .or. neggi(g) >= 200.
!insectmass(8,1,g) == 0. .and. neggi(g) > 0. .and. insectmass(9,1,g)==0.
!first condition is to keep development from rolling over until adult (s = 8) state progresses to 1, this is something like 15 days..too long? -hiro
!second condition is to ensure that rollover occurs when egg laying climatic conditions are satisfied- not sure if this makes sense or is necessary
!third condition ensures rollover only ensures once, as if it happened multiple times it would interfere with simple energy and state parametrization
												!allow eggs to start development
												insectmass(9,:,g) = insectmass(9,:,g) + eggmass(:,g)
												insectenergy(9,:,g) = 2.1571
												insectstate(9,:,:) = 0.!hiro helps nan problem by initialising value
												eggmass(:,g) = 0.
