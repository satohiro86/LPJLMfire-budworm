...
do g = 1,nisex
		do s = 1,nistage
				...
				case(1)  !L20, hiro 
						if(d<200 .and. (tmean >= Tb(s,g) .and. tmean <= Tm(s,g)))then
								tau = (tmean - Tb(s,g)) / (Tm(s,g) - Tb(s,g))
								rate(s,g) = b1(s,g) * ((1/ (1 + exp((b2(s,g) - (b3(s,g) * tau))))) - (exp (((tau - 1)/b4(s,g)))))
						!if(d>200) rate(s,g)=0
				end if
    ...
    do s = 1,nistage
      ...
      if (ns > nistage) ns = 1
						...
						if (state(s,g,r) >= 1.) then  !stage completed
								...
								state(s,g,r) = 0.
								!energy(s,g,r) = 0. !hiro edit to allow energy rollover
...
if (bavard) then
  if (any(mass > 0.)) then
    ...
    write(diagfid,10)'mass_f',mass(:,1,1)
    write(diagfid,10)'statef',state(:,1,1)
    write(diagfid,10)'rate_f',rate(:,1)
    write(diagfid,10)'energyf',energy(:,1,1)
  end if
...
!------------------------
 ...
 if (bavard) then
	write(diagfid,*)'tmean',tmean
	write(diagfid,*)'massegg',massegg(1)
	write(diagfid,*)'neggi',neggi
	write(diagfid,*)'oogenesis',oogenesis
	write(diagfid,*)'nfemales',nfemales
	write(diagfid,*)'neggs',nfemales
 end if

nfemales = famass / unitmass !sets number of female by mass in s = 8 stage

neggs = nfemales * (200. - neggi) !sets total number of eggs in females taking into account eggs already laid that year? Initialized at 0 every year it seems.

if (tmean > 10. .and. tmean <= 25.) then 

		oogenesis = neggs * (0.035 * tmean - 0.32) !allows oogenesis, which I think is more like oviposition. I suppose eggs are laid over many days? What stops it?
...
massegg(:) = massegg(:) + 0.5 * oogenesis * eggmass !allows eggmass to increase for each round of oogenesis/oviposition

neggi = neggi + oogenesis / nfemales !increase the number of 'eggs already laid' each round
